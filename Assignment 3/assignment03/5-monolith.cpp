...
void init() {
    ...
    //    the_view->camera_position = glm::vec3(0, 0, 3);
        ...
        //part 5 stuff
        auto* monolith = new cs4722::artifact();
        monolith->the_shape = shapes[0];
        monolith->world_transform.scale = glm::vec3(.5*pc_height, 4*pc_height, .125*pc_height);

        glm::vec3 MonPos(1, .1, .1);
        MonPos = glm::normalize(MonPos);
        MonPos = MonPos * (asteroid_radius + (2 * pc_height));
        MonPos += asteroid_center;
        monolith->world_transform.translate = MonPos;

        auto McToP = MonPos - asteroid_center;

        monolith->world_transform.rotation_axis = glm::vec3(McToP.z, 0, -McToP.x);
        monolith->world_transform.rotation_angle = glm::acos(MonPos.y / glm::length(MonPos));
        monolith->world_transform.rotation_center = glm::vec3(0, 0, 0);

        monolith->surface_material.diffuse_color = cs4722::x11::gray30;

        artf_list.push_back(monolith);
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        auto position = glm::sphericalRand(asteroid_radius + height / 2) + asteroid_center;wwwwww
...
void general_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    the_view->camera_left = glm::normalize(the_view->camera_left - t);
