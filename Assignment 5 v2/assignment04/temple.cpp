...
static GLint u_transform = 10;
static GLint u_n_transform = 7;
static GLint u_m_transform = 8;
static GLint u_sampler = 2;
static GLint u_surface_effect = 5;
static GLint u_diffuse_color = 4;
const static GLuint u_samplerC = 3;
const static GLuint u_camera_position = 9;
...
//                       glm::vec3(-0.146291,0.195185,1.3214));
    ...
    cs4722::init_texture_computed(5, 12);
    cs4722::init_cube_texture_from_path("../mediaA5/fjaderholmarna-x", 13, "png");
    //end
    ...
    artfb->texture_unit = 13;
    artfb->surface_effect = 5;

    artifact_list.push_back(artfb);

    //REFLECTING BLOCK HERE
    cs4722::shape* b1 = new cs4722::block();
    auto* obj1 = new cs4722::artifact_rotating();
    obj1->the_shape = (b1);
    obj1->world_transform.translate = (glm::vec3(number/2, number/2, 3*height));
    ...
    obj1->surface_effect = 6;
    obj1->texture_unit = 13;
...
void render()
{
    ...
    for (auto artf : artifact_list) {
        ...
        //changes 6
        if(artf->surface_effect == 6 || artf->surface_effect == 5) {
            glUniform1i(u_samplerC, artf->texture_unit);
        }
        else if (artf->surface_effect != 6 || artf->surface_effect != 5){
